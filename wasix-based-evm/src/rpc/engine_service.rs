    ...
    async fn forkchoice_updated(
        ...
        let (status, payload_id) = if status.status == PayloadStatusEnum::Valid || (status.status == PayloadStatusEnum::Syncing && payload_attributes.is_some()) {
            if let Some(attr) = payload_attributes {
                let mut mut_storage_snapshot = storage_snapshot;
                match self.build_new_payload(mut_storage_snapshot.as_any_mut().downcast_mut::<InMemoryStorage>().unwrap(), forkchoice_state.head_block_hash, attr, &status).await {
                    Ok(id) => (status, id),
