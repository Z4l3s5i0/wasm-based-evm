...
# Manual argument parsing to support --registry and --tag consistently with other scripts
...
if ([string]::IsNullOrWhiteSpace($Registry)) {
    Write-Host "Usage: .\push_windows.ps1 --registry <registry_url> [--tag <tag>]" -ForegroundColor Yellow
    Write-Host "Error: --registry is required." -ForegroundColor Red
    exit 1
}
