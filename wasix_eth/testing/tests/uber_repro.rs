...
async fn test_uber_gas_mismatch_repro() {
    ...
    let (executed_block1, receipts1) = node.engine.execution.execute_block_with_commit(block1, true).expect("Block 1 execution failed");
    ...
    // Calculate contract address manually: keccak256(rlp([sender, nonce]))
    ...
    // 5. Call checkDistance() in Block 2 with MISMATCHED gas_used
    ...
    let mut header2 = Header {
        ...
        ommers_hash: EMPTY_OMMER_ROOT_HASH,
        beneficiary: Address::ZERO,
        state_root: B256::repeat_byte(0x01), // Force validation mode
        ...
        gas_used: 210000, // THE MISMATCH: user reports 210000 expected
        timestamp: 2,
        extra_data: Bytes::new(),
        mix_hash: B256::ZERO,
        nonce: B64::ZERO,
    ...
    // We need to manually insert the parent state root mapping if we use a fake state root
    node.write_provider.insert_block_hash(header2.state_root, 1).unwrap();
    ...
    info!("Executing block 2 (call checkDistance) with mismatched gas_used");
    let result = node.engine.execution.execute_block_with_commit(block2, true);
    
    match result {
        Ok(_) => panic!("Block 2 execution should have failed with gas mismatch"),
        Err(e) => {
            info!("Caught expected error: {}", e);
            assert!(e.to_string().contains("Gas used mismatch"), "Error should be gas mismatch, got: {}", e);
            assert!(e.to_string().contains("expected 210000"), "Error should mention expected 210000");
        }
    }
}
