use wasix_eth_types::{Transaction as TxEnvelope, TxLegacy, SignableTransaction, Address, TxEip1559};
...
impl AccountManager {
    ...
    pub async fn sign_transaction_1559(&self, from: &Address, tx: TxEip1559) -> RpcResult<TxEnvelope> {
        let signer = self.signers.get(from)
            .ok_or_else(|| RpcError::AccountNotFound(*from))?;
    ...
    pub async fn sign(&self, from: &Address, message: &[u8]) -> RpcResult<Signature> {
        let signer = self.signers.get(from)
            .ok_or_else(|| RpcError::AccountNotFound(*from))?;
